  ...
  return (
    <div className="p-6 flex flex-col border-2 shadow-md justify-around items-center" style={{height: '500px'}}>
        ...
        <div className="flex flex-row w-11/12 justify-between items-center">
          <button onClick={addToCart} className="flex justify-center items-center w-44  h-10 bg-color5 text-white px-6"><span className="mr-4"><FaShoppingCart /></span><span>Add to cart</span></button>
          <button onClick={addToWishList} className="text-red-600 text-xl h-10 w-10 flex justify-center items-center bg-white border-2 rounded-sm"><FaRegHeart /></button>
        </div>
    </div>
  )
}
